...
- **RIN spectrum.** Burd 2023 makes the class-A claim qualitatively but does not publish a RIN spectrum. [Burd 2016 §2](https://github.com/uwarring82/mg-plus-uv-chain/blob/main/data/literature/Burd2016/extracted.yaml) reports `< -75 dBc/Hz` above 10 kHz at 280 nm. Comparison to Friedenauer 2006's `< -130 dB/Hz` at 2-10 MHz is partial (different frequency bands). Two distinct quantities are at play: the **input budget** is the seed's frequency-noise spectral density above the ~18 kHz doubling-cavity lock bandwidth, while the **downstream observable** is the UV RIN the doubling cavities produce by converting that frequency noise to amplitude noise. Neither is yet measured for the candidate VECSEL build.
