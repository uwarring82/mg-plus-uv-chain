...
| LBO loaded piezo resonance (literature) | ≈ 18 kHz; not a measured closed-loop bandwidth | Friedenauer 2006, §3 ([`Friedenauer2006::LBO_M2_piezo_resonance_loaded`](https://github.com/uwarring82/mg-plus-uv-chain/blob/main/data/literature/Friedenauer2006/extracted.yaml)) |
...
The seed-linewidth figures are **proxies** for the frequency-noise assessment. A linewidth larger than the literature piezo resonance does not by itself establish suitability or unsuitability: the residual noise depends on the actual cavity/servo response. That response and the closed-loop bandwidth require measurement for each assembled system; the 18 kHz resonance is not assigned to the planned locks. See the [planned cavity-lock electronics](shg-designs.html#cavity-lock-electronics) for the controller reference and its limits. The atomic linewidth and the iodine reference concern cooling/detection resolution and long-term locking, respectively.
...
- **RIN spectrum.** Burd 2023 makes the class-A claim qualitatively but does not publish a RIN spectrum. [Burd 2016 §2](https://github.com/uwarring82/mg-plus-uv-chain/blob/main/data/literature/Burd2016/extracted.yaml) reports `< -75 dBc/Hz` above 10 kHz at 280 nm. Comparison to Friedenauer 2006's `< -130 dB/Hz` at 2-10 MHz is partial (different frequency bands). Two distinct quantities are at play: the **input budget** is the seed's frequency-noise spectral density weighted by the measured cavity/servo response, while the **downstream observable** is the resulting contribution to UV RIN. Neither that response nor the candidate build's UV RIN has been measured here.
...
- [Hardware status — VECSEL seed fleet](../hardware-status.html) — the operational snapshot: what is built and what is open across the in-house fleet (#2 "Heidi" 1141 nm, the 1118 nm units, V#5, NeXt).
