...
- **2026-05-04 (assistant under steward direction, DRAFT).** Targeted re-read of §3 to fill gaps surfaced by [`/notebooks/2026-05-01-friedenauer-bk-recalculation.py`](../../../notebooks/2026-05-01-friedenauer-bk-recalculation.py) §8:
  ...
  - **Cavity locking and servo bandwidth** — added `cavity_locking_scheme` (Hänsch-Couillaud, ref [6]) used for both cavities, plus `LBO_M2_piezo_model` (Thorlabs AE020304D04 stacked piezo on a lead disk) and `LBO_M2_piezo_resonance_loaded` ≈ 18 kHz. *Correction (RC-04, 2026-09-16):* that value is the loaded piezo resonance as printed in §3 (printed p. 372); it is **not** a measured servo or closed-loop bandwidth. The "bounds the achievable servo bandwidth" gloss recorded in this pass was an extraction-added inference and is withdrawn — the paper reports no lock-bandwidth measurement.
