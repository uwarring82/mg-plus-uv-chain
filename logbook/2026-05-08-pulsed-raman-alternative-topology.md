...
- **Multi-level ²⁵Mg⁺ pulse-train model — Phase 4 acceptance prerequisite.** A scaling argument based on `Γ_sc ∝ 1/Δ²` and `Ω_R ∝ I/Δ` is *not* sufficient to accept this candidate. Phase 4 scoring requires a multi-level model of ²⁵Mg⁺ ³S₁/₂ → {³P₁/₂, ³P₃/₂} interacting with the comb spectral envelope (~ 3 THz transform-limited at 100 fs, comparable to the fine-structure splitting), computing per-gate Ω_R and Γ_sc with both fine-structure components addressed simultaneously. Until that model lands and reproduces a relevant Monroe-group Yb⁺ benchmark before being applied to ²⁵Mg⁺, this sketch is below gate-level acceptance.
- **Council-3 deliberation requests** if and when promotion to architecture-comparison candidate is desired: the CHARTER §3 scope conflict and the multi-operating-point question both deserve explicit clarification.
