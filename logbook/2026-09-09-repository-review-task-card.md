...
- **Colab pin maintenance — accountable owner: Ulrich Warring.** Add a CI check that all tutorial `REFERENCE_REVISION` pins agree and compare their pinned runtime dependencies with the current teaching inputs: imported `src/` modules, tutorial YAML defaults, installation metadata, and any constraints actually consumed. A changed dependency must fail the check unless accompanied by a reviewed pin update or a dated steward decision to retain the earlier teaching baseline. Documentation-only commits need no pin bump. After an update, run [check_colab_tutorials.py](../scripts/check_colab_tutorials.py), regenerate the tutorials and refresh affected manifests. Demonstrate failure for an injected dependency change and success for an unrelated documentation change. This enforcement is **planned under RC-06**, not already supplied by the runtime check, which verifies the selected revision rather than its freshness.
...
**Progress 2026-09-09:** README status and its existing/planned path split are corrected; dossier index rows 008/009/011/012 now match their existing detail sections (9/15 past scaffold, separate from 22 extraction files). Evidence and validation are in the [RC-09 follow-up](2026-09-09-public-record-corrections.md#rc-09-follow-up--readme-status-and-existing-paths). Wider link/schema/status work remains open. Dated notices now link all three constraint drafts to RC-02/03/04; their equations, tables and existing status declarations are unchanged. The steward reports 13 sampled live `.html` targets returning HTTP 200, resolving those sampled repo-relative warnings as Jekyll path conventions rather than site defects; the wider link audit remains open.
